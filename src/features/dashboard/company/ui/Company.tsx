...
import { ICompanyStats, IDriver, IDriverVehicle, IVehicle, IVehicleBodyType, IVehicleMake, IVehicleModel, PaginatedResponse } from "../types";
...
export default function Company() {
  ...
  const { getInfoStatistics, getDrivers, getVehicles, getDriverVehicles } = useCompanyServices();
  const { getVehicleTypes, getVehicleMake, getVehicleModel, getVehicleBodyTypes } = useCompanyServices();
  ...
  const [driverVehicles, setDriverVehicles] = useState<PaginatedResponse<IDriverVehicle>>();
  const [vehicles, setVehicles] = useState<PaginatedResponse<IVehicle>>();
  const [vehicleMake, setVehicleMake] = useState<PaginatedResponse<IVehicleMake>>();
  const [vehicleModels, setVehicleModels] = useState<PaginatedResponse<IVehicleModel>>();
  const [vehicleBodyTypes, setVehicleBodyTypes] = useState<PaginatedResponse<IVehicleBodyType>>();
  ...
  return (
    <div>
      <Group justify="space-between">
        ...
        <Group>
          <DriverFormModal fetchData={fetchDrivers} switchTab={() => {
            setActiveTab("first")
            searchParams.delete('tab');
            setSearchParams(searchParams);
          }} vehicles={driverVehicles?.results ?? []} />
          <VehicleFormModal fetchData={fetchVehicles} switchTab={() => {
            setActiveTab("second")
            searchParams.set('tab', 'second');
            setSearchParams(searchParams);
           
          }} vehicles={vehicles?.results ?? []} vehicleMake={vehicleMake?.results??[]} vehicleBodyTypes={vehicleBodyTypes?.results??[]} vehicleModels={vehicleModels?.results??[]} />
        </Group>
      </Group>
      <Space h="md" />
      ...
      <Tabs keepMounted={false} value={activeTab} onChange={setActiveTab} variant="pills">
        <div style={{ backgroundColor: "#EDF0F6", borderRadius: "7px" }} className="w-[222px] p-[2px]">
